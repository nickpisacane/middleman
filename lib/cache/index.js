...
Cache.prototype._del = function(key) {
  var self = this;
  // This is called before LRU actually drops the key, by "protecting" the key,
  // it can be fully deleted from the LRU without multiple delete calls to the
  // store.
  self._protect(key);
  self._lruCache.del(key);
  self.store.del(key)
