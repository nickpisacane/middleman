 ...
 * @param {String} options.target URI for proxy
 * @param {Object} [options.setHeaders = {}]  Headers to set for proxied request
 * @param {Array|String} [options.cacheMethods = 'any'] HTTP methods to cache for
 * @param {Number} [options.maxAge = Infinity] Cache age
 * @param {Function} [options.cacheKey] Default is {something}
 * @constructor
 * @public
 */
 ...
 * @public
 */
Middleman.prototype.handle = function(req, res) {
  var self = this;
  var urlObj = req.__urlObj = Url.parse(req.url);
  var key = self._createKey(req, urlObj);
  self.cache.get(key)
    .then(function(data) {
      //console.log('data for key: '+ key + ' data: ', data);
      if (!data) {
        self.emit('proxy request', req);
        var proxy = self._proxy(req);
        proxy.on('error', function(err) {
          self.emit('error', err);
          self._errorResponse(res);
        });
        self.emit('proxy response', res);
        proxy.pipe(res);
      } else {
        self.emit('cache request', req);
        self._send(data.value, res);
      }
    })
    .catch(function(err) {
      self.emit('error', err);
...
/**
 * Convenience method, returns `handle` with instance context
 * @return {Function}
 * @public
 */
...
/**
 * Convenience method, creates an instance of http.Server.
 * @return {http.Server}
 * @public
 */
...
Middleman.prototype.createKey = function(fn) {
  if (typeof fn !== 'function') {
    throw new TypeError('fn must be of Function type');
  }
  this._createKey = fn;
  return this;
};
 ...
 * @private
 */
Middleman.prototype._send = function(cachedResponse, res) {
  if (!(cachedResponse instanceof CachedResponse)) {
    try {
      ...
      return this._errorResponse(res);
    }
  }
  this.emit('cache response', res);
 ...
 * @private
 */
Middleman.prototype._errorResponse = function(res) {
...
Middleman.prototype._proxy = function(req) {
  ...
  var key = self._createKey(req, req.__urlObj);
...
Middleman.prototype._createUri = function(req) {
  return urlJoin(this.settings.target, req.__urlObj.path);
};
 ...
 * of parsing a JSON-serialized buffer, meaning it be an object with two properties
 * `type`, and `data`.
 *
